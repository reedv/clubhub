...
Template.Club_Profile_Page.onCreated(function onCreated() {
  this.autorun(() => {
  ...
  this.context = ClubsSchema.namedContext('Club_Profile_Page');
});

// useful thing to note, all Collection docs. have a _id key that is uniq. to that doc
Template.Club_Profile_Page.helpers({
  clubDataField(fieldVal) {
    ...
    const club = Clubs.findOne(FlowRouter.getParam('_id'));
...
Template.Club_Profile_Page.onRendered(function enableSemantic() {
  const instance = this;
...
Template.Club_Profile_Page.events({
